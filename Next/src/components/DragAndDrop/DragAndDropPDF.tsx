...
const DragAndDropPDF = ({ disabled, onChange, uploadedFilesData }: Props) => {
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      console.log({ files });
      try {
        const resFile = await saveUploadFilesData(submittedFormId as string, {
          files: acceptedFiles,
        });
        // TODO:
        ...
        //   size: file.size,
        // }));
        console.log({ resFile: JSON.parse(JSON.stringify(resFile)) })
  ...
  const removeAllChips = async () => {
    ...
    setDisplayFiles([]);
    try {
      await removeAllUploadedFiles(submittedFormId as string);
    } catch (error) {
      console.log({ error });
    }
  };
